...
step=1
end_epoch=19
model_folder="experiment/preprocess/checkpoints/"
while test $start_epoch -le $end_epoch; do
  ...
  if [ -f ${model_folder}${model_name} ]; then 
    ...
    srun --mpi=pmi2 --job-name $1 --partition=HA_vechicle -n1 --gres=gpu:8 --ntasks-per-node=8 \
        python -u eval_lfw.py --loss_type a-softmax --ckpt experiment/preprocess --lfw /mnt/lustre/xujingyi/sphereface/preprocess/result/lfw-112X96 --model=$start_epoch
    ((start_epoch+=step))
  else
    echo "sleep"
    sleep 30m
